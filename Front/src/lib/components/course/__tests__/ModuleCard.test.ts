...
describe('ModuleCard', () => {
	...
	describe('Actions - Public View', () => {
		...
		it('should emit "view" event when "Ver" button is clicked', async () => {
			const { component } = render(ModuleCard, { module: mockModule, showActions: false });

			const viewEventSpy = vi.fn();
			component.$on('view', viewEventSpy);

			const viewButton = screen.getByRole('button', { name: /ver/i });
			await fireEvent.click(viewButton);

			expect(viewEventSpy).toHaveBeenCalledWith(
	...
	describe('Actions - Admin View', () => {
		...
		it('should emit "edit" event when edit button is clicked', async () => {
			const { component } = render(ModuleCard, { module: mockModule, showActions: true });

			const editEventSpy = vi.fn();
			component.$on('edit', editEventSpy);

			const editButton = screen.getByTitle(/editar módulo/i);
			await fireEvent.click(editButton);

			expect(editEventSpy).toHaveBeenCalledWith(
				expect.objectContaining({
					detail: 'module-1'
				})
			);
