	...
	<div class="modal-overlay" on:click={handleBackdropClick}>
		<div class="modal-content post-form-modal">
			<div class="modal-header">
				<h3>{modalTitle}</h3>
				<button class="btn-close" on:click={handleClose} disabled={isLoading}>
									...
									>
										<div class="element-header">
											...
											<div class="element-actions">
												<button type="button" class="btn-icon" on:click={() => toggleEditElement(index)} disabled={isLoading}>
												...
												<button type="button" class="btn-icon btn-danger" on:click={() => removeElement(index)} disabled={isLoading}>
													<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<style>
	...
	.media-upload input[type="file"]:hover:not(:disabled) {
		border-color: var(--color-primary);
