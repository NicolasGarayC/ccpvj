...
describe('Pagination Component', () => {
	...
	describe('Edge Cases', () => {
		...
		it('should update when currentPage prop changes', async () => {
			...
			let currentPageButton = screen.getByLabelText('Página 1');
			expect(currentPageButton).toHaveClass('active');

			await component.$set({ currentPage: 3, hasPrevious: true });

			currentPageButton = screen.getByLabelText('Página 3');
			expect(currentPageButton).toHaveClass('active');
		});
		...
		it('should update when totalPages prop changes', async () => {
			...
			await component.$set({ totalPages: 10 });
