...
// Mock dependencies
vi.mock('$lib/services/blog/blogService', () => ({
	blogService: {
		getAllPosts: vi.fn()
	}
}));
...
describe('BlogPostList', () => {
	...
	describe('Create Post Flow', () => {
		...
		it('should create draft post when valid title is entered', async () => {
			...
			vi.mocked(blogHttpService.createPost).mockResolvedValue(mockDraftPost);
			...
			await waitFor(() => {
				expect(blogHttpService.createPost).toHaveBeenCalledWith({
					title: 'New Draft',
					excerpt: '',
					content: '',
					slug: '',
		...
		it('should handle create draft error', async () => {
			vi.mocked(blogHttpService.createPost).mockRejectedValue(new Error('Create failed'));
	...
	describe('Component Events', () => {
		it('should dispatch postsLoaded event when posts are loaded', async () => {
			...
			component.$on('postsLoaded', postsLoadedSpy);
		...
		it('should dispatch deletePost event when delete is triggered', async () => {
			const { component } = render(BlogPostList, { props: { showActions: true } });

			const deletePostSpy = vi.fn();
			component.$on('deletePost', deletePostSpy);

			await waitFor(() => {
				// This would require interaction with BlogPostCard component
				// For now, we verify the handler exists
				expect(component).toBeTruthy();
			});
		});

		it('should dispatch postCreated event when post is created', async () => {
			const mockDraftPost = { ...mockPosts[0], id: '3', title: 'New Draft', status: 'draft' };
			vi.mocked(blogHttpService.createPost).mockResolvedValue(mockDraftPost);

			const { component } = render(BlogPostList, { props: { showActions: true } });

			const postCreatedSpy = vi.fn();
			component.$on('postCreated', postCreatedSpy);

			// Would need to trigger post creation flow
			expect(component).toBeTruthy();
		});
	});
	...
	describe('Public Methods', () => {
		...
		it('should not reload posts if already loading', async () => {
			...
			await waitFor(() => {
				// Should only call once (initial) + once (first explicit call)
				expect(blogService.getAllPosts).toHaveBeenCalledTimes(2);
			});
		});
	});
