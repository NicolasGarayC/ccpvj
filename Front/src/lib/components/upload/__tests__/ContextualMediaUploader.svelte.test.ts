...
describe('ContextualMediaUploader', () => {
	...
	describe('File Validation', () => {
		...
		it('should dispatch uploadError event when validation fails', async () => {
			vi.mocked(contextualUploadService.validateFile).mockReturnValue({
				isValid: false,
				error: 'Invalid file'
			});

			const { component, container } = render(ContextualMediaUploader, {
				props: { context: 'course', contentId: 'course-1' }
			});

			const errorHandler = vi.fn();
			component.$on('uploadError', errorHandler);
	...
	describe('Course Image Upload', () => {
		it('should upload course image successfully', async () => {
			...
			component.$on('uploadSuccess', successHandler);
		...
		it('should dispatch uploadStart event', async () => {
			vi.mocked(contextualUploadService.uploadCourseImage).mockResolvedValue(mockUploadResult);
			...
			component.$on('uploadStart', startHandler);
		...
		it('should dispatch uploadProgress events', async () => {
			vi.mocked(contextualUploadService.uploadCourseImage).mockResolvedValue(mockUploadResult);
			...
			component.$on('uploadProgress', progressHandler);
	...
	describe('Upload Error Handling', () => {
		...
		it('should dispatch uploadError event on failure', async () => {
			vi.mocked(contextualUploadService.uploadCourseImage).mockRejectedValue(
	...
	describe('Current Media Display', () => {
		...
		it('should dispatch mediaRemoved event when remove is clicked', async () => {
			const { component } = render(ContextualMediaUploader, {
				props: { context: 'course', contentId: 'course-1', currentMedia: '/uploads/current.jpg' }
			});

			const removeHandler = vi.fn();
			component.$on('mediaRemoved', removeHandler);
	...
	describe('Drag and Drop', () => {
		...
		it('should handle drag leave', async () => {
			...
			await fireEvent(uploadArea, dragOverEvent);

			const dragLeaveEvent = new DragEvent('dragleave', { bubbles: true });
			await fireEvent(uploadArea, dragLeaveEvent);
