...
describe('MaterialApoyoCard', () => {
	...
	describe('Actions - Admin View', () => {
		...
		it('should open delete modal when "Eliminar" is clicked', async () => {
			...
			expect(screen.getByRole('heading', { name: /eliminar material de apoyo/i })).toBeInTheDocument();
			expect(screen.getAllByText(/matemáticas básicas/i).length).toBeGreaterThan(0);
		});
	...
	describe('Date Formatting', () => {
		it('should format Unix timestamp correctly', () => {
			...
			const dateText = screen.getByText(/creado:/i).textContent ?? '';
