...
import type { EventDetail } from '$lib/application/services/calendar/CalendarService';
...
describe('EventForm', () => {
	...
	describe('Validation', () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText('Para eventos recurrentes, debes especificar hasta cuándo se repite')
			).toBeInTheDocument();
		});
		});
	...
	describe('Submit and Cancel', () => {
		...
		it('should dispatch cancel event when cancel is clicked', async () => {
			...
			render(EventForm, { props: { ...defaultProps, onCancelCallback: cancelSpy } });
	...
	describe('Edge Cases', () => {
		...
		it('should handle very long title', async () => {
			...
			// jsdom does not enforce maxlength, but attribute should be present
		...
		it('should handle very long description', async () => {
			...
			expect((descriptionTextarea as HTMLTextAreaElement).maxLength).toBe(1000);
		});
	});
