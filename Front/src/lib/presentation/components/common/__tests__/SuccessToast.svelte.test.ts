...
describe('SuccessToast Component', () => {
	...
	describe('Auto-close Behavior', () => {
		it('should auto-close after default duration (3000ms)', async () => {
			...
			render(SuccessToast, {
				props: { visible: true, message: 'Success!', onCloseCallback: closeHandler }
			});
		...
		it('should auto-close after custom duration', async () => {
			const closeHandler = vi.fn();
			render(SuccessToast, {
				props: { visible: true, message: 'Success!', duration: 5000, onCloseCallback: closeHandler }
			});
		...
		it('should not auto-close before duration is complete', async () => {
			const closeHandler = vi.fn();
			render(SuccessToast, {
				props: { visible: true, message: 'Success!', duration: 3000, onCloseCallback: closeHandler }
			});
		...
		it('should reset timer when visible changes from false to true', async () => {
			...
			const { rerender } = render(SuccessToast, {
				props: { visible: false, message: 'Success!', duration: 3000, onCloseCallback: closeHandler }
			});

			// Make visible
			await rerender({ visible: true, message: 'Success!', duration: 3000, onCloseCallback: closeHandler });
	...
	describe('Manual Close Actions', () => {
		...
		it('should close when toast is clicked', async () => {
			...
			const toast = screen.getByRole('button', { name: 'Ocultar notificación' });
	...
	describe('Edge Cases', () => {
		it('should handle duration of 0', async () => {
			const closeHandler = vi.fn();
			render(SuccessToast, {
				props: { visible: true, message: 'Success!', duration: 0, onCloseCallback: closeHandler }
			});
		...
		it('should handle very long duration', async () => {
			const closeHandler = vi.fn();
			render(SuccessToast, {
				props: { visible: true, message: 'Success!', duration: 100000, onCloseCallback: closeHandler }
			});
		...
		it('should update message when prop changes', async () => {
			...
			await rerender({ visible: true, message: 'Second message' });
		...
		it('should hide when visible is set to false', async () => {
			...
			await rerender({ visible: false, message: 'Success!' });
