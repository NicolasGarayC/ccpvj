...
describe('BlogEventRelation', () => {
	...
	describe('Search Functionality', () => {
		it('should filter events by search term', async () => {
			...
			await waitFor(() => {
				...
				expect(dropdownScope.getByText('Conferencia de Tecnología')).toBeInTheDocument();
				expect(dropdownScope.queryByText('Taller de Programación')).not.toBeInTheDocument();
			});
		});
		...
		it('should show all events when search is cleared', async () => {
			...
			await waitFor(() => {
				...
				expect(dropdown).toBeTruthy();
				const dropdownScope = within(dropdown as HTMLElement);
				expect(dropdownScope.getByText('Taller de Programación')).toBeInTheDocument();
			});
		});
		...
		it('should show empty state when no events match search', async () => {
			...
			await waitFor(() => {
				...
				expect(dropdown).toBeTruthy();
				const dropdownScope = within(dropdown as HTMLElement);
				expect(dropdownScope.getByText('No se encontraron eventos')).toBeInTheDocument();
			});
		});
	...
	describe('Selected Events Display', () => {
		...
		it('should display event details in selected events list', async () => {
			...
			await waitFor(() => {
				const selectedSection = document.querySelector('.selected-events');
				expect(selectedSection).toBeTruthy();
				const cards = selectedSection?.querySelectorAll('.event-card') ?? [];
				expect(cards.length).toBe(1);
				const scope = within(selectedSection as HTMLElement);
				expect(scope.getByText('Taller de Programación')).toBeInTheDocument();
			});
		});
		...
		it('should show event location when available', async () => {
			...
			await waitFor(() => {
				...
				expect(selectedSection).toBeTruthy();
				const scope = within(selectedSection as HTMLElement);
				expect(scope.getByText(/📍 Sala A/)).toBeInTheDocument();
			});
		});
		...
		it('should format event date correctly', async () => {
			...
			await waitFor(() => {
				...
				expect(selectedSection).toBeTruthy();
				const scope = within(selectedSection as HTMLElement);
				expect(scope.getByText(/📅.*15.*jun/i)).toBeInTheDocument();
			});
		});
