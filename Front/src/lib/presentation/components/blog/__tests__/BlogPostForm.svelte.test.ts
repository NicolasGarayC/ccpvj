...
const {
	...
	updatePostMock,
	getArticleByIdMock,
	getElementsByBlogPostIdMock,
	createElementsInBatchMock,
	deleteElementMock,
	cleanupOrphanFilesMock,
	getEventsByBlogPostMock,
	...
	getElementsByBlogPostIdMock: vi.fn(() => Promise.resolve([])),
	createElementsInBatchMock: vi.fn(() => Promise.resolve()),
	deleteElementMock: vi.fn(() => Promise.resolve()),
	cleanupOrphanFilesMock: vi.fn(() => Promise.resolve()),
	getEventsByBlogPostMock: vi.fn(() => Promise.resolve([])),
	getUpcomingEventsMock: vi.fn(() => Promise.resolve([])),
...
vi.mock('$lib/application/services/blog/blogPostElementService', () => ({
	blogPostElementService: {
		getElementsByBlogPostId: getElementsByBlogPostIdMock,
		createElementsInBatch: createElementsInBatchMock,
...
vi.mock('$lib/application/services/upload/ContextualUploadService', () => ({
	contextualUploadService: {
		cleanupOrphanFiles: cleanupOrphanFilesMock
	}
}));

vi.mock('$lib/application/services/calendar/CalendarService', () => ({
	calendarService: {
		getEventsByBlogPost: getEventsByBlogPostMock,
...
describe('BlogPostForm', () => {
	...
	beforeEach(() => {
		...
		getArticleByIdMock.mockResolvedValue(mockPost);
	});
	...
	describe('Element Management', () => {
		...
		it('should add text element when text button is clicked', async () => {
			...
			const textArea = screen.getByPlaceholderText(/Ingresa el texto/i) as HTMLTextAreaElement;
	...
	describe('Close Behavior', () => {
		...
		it('should dispatch close event when cancel is clicked', async () => {
			...
			render(BlogPostForm, { props: { visible: true, post: null, onClose: closeSpy } });
		...
		it('should dispatch close event when X button is clicked', async () => {
			const closeSpy = vi.fn();
			const { container } = render(BlogPostForm, {
				props: { visible: true, post: null, onClose: closeSpy }
			});
