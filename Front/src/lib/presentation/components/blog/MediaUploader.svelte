<script lang="ts">
  ...
  function isImage(url: string) {
    return /^data:image\//i.test(url) || url.match(/\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i);
  }

  function isVideo(url: string) {
    return /^data:video\//i.test(url) || url.match(/\.(mp4|webm|mov|avi|mkv)$/i);
  }

  function isAudio(url: string) {
    return /^data:audio\//i.test(url) || url.match(/\.(mp3|wav|ogg|flac|aac)$/i);
  }

  function isPdf(url: string) {
    return /^data:application\/pdf/i.test(url) || url.match(/\.pdf$/i);
  }

  function isDocument(url: string) {
    return (
      isPdf(url) ||
      url.match(/\.(doc|docx|xls|xlsx|ppt|pptx)$/i) != null
    );
  }
