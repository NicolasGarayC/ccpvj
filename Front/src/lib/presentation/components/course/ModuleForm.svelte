...
import { onMount, createEventDispatcher } from 'svelte';
	...
	export let onSuccess:
		| ((
				detail:
					| { type: 'update'; id: string; data: UpdateModuleDto }
					| { type: 'create'; module: ModuleSummaryDto }
		  ) => void)
		| undefined = undefined;
...
$: {
	if (module && module.id !== previousModuleId) {
		formData = {
			title: module.title || '',
			description: module.description || '',
			orderNumber: module.orderNumber ?? 1
	...
	} else if (!module && previousModuleId) {
		formData = {
			title: '',
			description: '',
			orderNumber: existingModules.length + 1
		};
		formErrors = {};
		...
		submitting = true;
			...
			try {
				if (isEditing && module) {
					...
					const detail = { type: 'update', id: module.id, data: updateData } as const;
					onSuccess?.(detail);
					...
					const detail = { type: 'create', module: newModule } as const;
					onSuccess?.(detail);
					dispatch('success', detail);
				}
			} catch (error) {
					...
					<div class="form-group">
						...
						<div class="order-input-container">
							<input
								...
								min="1"
								max="{Math.max(existingModules.length + 1, module?.orderNumber ?? 1)}"
								disabled={submitting || loading}
								required
							/>
