...
import { modulePostService, type PostDetail } from '$lib/application/services/material-apoyo/ModulePostService';
	...
	export let onPostsLoaded: ((posts: PostDetail[]) => void) | undefined = undefined;
	export let onViewPost: ((post: PostDetail) => void) | undefined = undefined;
	export let onEditPost: ((post: PostDetail) => void) | undefined = undefined;
	export let onDeletePost: ((postId: string) => void) | undefined = undefined;
	export let onPostCreated: ((detail: unknown) => void) | undefined = undefined;
	export let onPostUpdated: ((detail: unknown) => void) | undefined = undefined;
	export let onPostsReordered: ((detail: { posts: PostDetail[] }) => void) | undefined = undefined;
	export let onCreatePost: (() => void) | undefined = undefined;
	export let onFormClose: (() => void) | undefined = undefined;

const dispatch = createEventDispatcher<{
	postsLoaded: PostDetail[];
	editPost: PostDetail;
	viewPost: PostDetail;
	deletePost: string;
	postCreated: unknown;
	postUpdated: unknown;
	postsReordered: { posts: PostDetail[] };
}>();
	...
	function extractPost(detail: unknown): PostDetail | null {
		if (detail && typeof detail === 'object') {
			const maybePost = (detail as Record<string, unknown>).post;
			if (maybePost && typeof maybePost === 'object') {
				return maybePost as PostDetail;
			}

			if ('id' in detail && 'orderNumber' in detail) {
	...
	function handlePostCreated(event: CustomEvent<unknown>) {
		const payload = event.detail;
		const newPost = extractPost(payload);
		...
		dispatch('postCreated', payload);
		handleFormClose();
	}

	function handlePostUpdated(event: CustomEvent<unknown>) {
		const payload = event.detail;
	...
	async function handleDrop(event: DragEvent, targetIndex: number) {
		...
		try {
			...
			onPostsReordered?.({ posts: newPosts });
		} catch (err) {
	...
	// Testing helpers
	export function __testHandlePostCreated(detail: unknown) {
		handlePostCreated({ detail } as CustomEvent<unknown>);
	}

	export function __testHandlePostUpdated(detail: unknown) {
		handlePostUpdated({ detail } as CustomEvent<unknown>);
	}

	export function __testHandleFormClose() {
		handleFormClose();
	}
