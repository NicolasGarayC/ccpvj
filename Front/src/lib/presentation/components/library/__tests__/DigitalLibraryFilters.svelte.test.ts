...
import type { LibrarySearchDto } from '$lib/application/services/library/DigitalLibraryService';
...
describe('DigitalLibraryFilters', () => {
	...
	describe('Filter Selection', () => {
		it('should dispatch filtersChange event when fileType is selected', async () => {
			...
			render(DigitalLibraryFilters, {
				props: { currentFilters: {}, onFiltersChange: filtersChangeHandler }
			});
		...
		const authorSelect = screen.getAllByRole('combobox').find((el) =>
			el.previousElementSibling?.textContent?.includes('Autor')
		...
		it('should handle tag toggle', async () => {
			...
			await waitFor(() => {
				expect(filtersChangeHandler).toHaveBeenCalledWith(
					expect.objectContaining({
						tags: expect.arrayContaining(['música'])
					})
				);
			});
		});
	...
	describe('Remove Filters', () => {
		it('should remove fileType filter when X button is clicked', async () => {
			...
			const filtersChangeHandler = vi.fn();
			render(DigitalLibraryFilters, {
				props: { currentFilters: filters, onFiltersChange: filtersChangeHandler }
			});
			...
			const latestFilters = filtersChangeHandler.mock.calls.at(-1)?.[0] as LibrarySearchDto;
			expect(latestFilters?.fileType).toBeUndefined();
		});
		...
		it('should remove category filter when X button is clicked', async () => {
			...
			const latestFilters = filtersChangeHandler.mock.calls.at(-1)?.[0] as LibrarySearchDto;
			expect(latestFilters && 'category' in latestFilters ? latestFilters.category : undefined).toBeUndefined();
		});
		...
		it('should remove tag when clicked', async () => {
			...
			const latestFilters = filtersChangeHandler.mock.calls.at(-1)?.[0] as LibrarySearchDto;
			expect(latestFilters?.tags ?? []).not.toContain('música');
		});
	});
	...
	describe('Clear All Filters', () => {
		it('should clear all filters when clear all button is clicked', async () => {
			...
			const latestFilters = filtersChangeHandler.mock.calls.at(-1)?.[0] as LibrarySearchDto | undefined;
			expect(Object.keys(latestFilters ?? {})).toHaveLength(0);
		});
