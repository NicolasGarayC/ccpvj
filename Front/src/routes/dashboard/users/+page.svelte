<script lang="ts">
  ...
  function handleEditUser(event: CustomEvent<{ user: User }>) {
    editingUser = event.detail.user;
    showCreateForm = true;
  }
...
{#if canManageUsers}
  <div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">{$t('dashboard.userManagement')}</h1>
        <p class="text-gray-600 mt-2">{$t('dashboard.userManagementSupport')}</p>
      </div>
      <button
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-auto">
          <div class="flex justify-between items-center p-6 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-900">
              {editingUser ? $t('dashboard.editUser') : $t('dashboard.createUser')}
            </h2>
            <button
              on:click={handleCloseForm}
              class="text-gray-400 hover:text-gray-600 transition-colors"
              aria-label={$t('users.form.closeModal')}
            >
...
{:else}
  <div class="container mx-auto px-4 py-8">
    <div class="text-center">
      <h1 class="text-2xl font-bold text-gray-900 mb-4">{$t('dashboard.accessDenied')}</h1>
      <p class="text-gray-600">{$t('dashboard.userManagementNoAccess')}</p>
      <a href="/dashboard" class="text-blue-600 hover:text-blue-800 mt-4 inline-block">
        {$t('dashboard.userManagementBack')}
      </a>
    </div>
  </div>
{/if}
