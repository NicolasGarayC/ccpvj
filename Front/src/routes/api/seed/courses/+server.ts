...
export const POST: RequestHandler = async ({ cookies }) => {
  try {
    // Validar que solo administradores puedan hacer seed
    ...
    if (!sessionResult.session || !sessionResult.user || sessionResult.user.role !== 'Administrador') {
      return error(403, 'Solo administradores pueden ejecutar el seed');
    }
    ...
    return json({
      success: true,
      message: 'Seed de cursos ejecutado exitosamente',
      data: {
        courses: 4,
        modules: 3,
        workItems: 5
    ...
    return error(500, 'Error ejecutando el seed de cursos');
  }
};